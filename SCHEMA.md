...
| `manually_edited` | `boolean` | no | `false` | True on rows whose payee/category was set directly rather than by a rule. **Nothing writes it any more**: the inline row editor that did was removed when tapping a payee/category became a drilldown query instead (`mobile/lib/drilldown.ts`). The column and the two server-side behaviours below stay, because rows written by that editor still carry it and must keep being left alone. `apply_category_rules()` skips these rows entirely, and the Plaid sync path (`syncItemTransactions.ts`) stops upserting new data into them — both so a direct edit isn't silently reverted by an unrelated rule change or a later Plaid update to the same transaction. |
...
Both HTTP jobs pass an explicit `timeout_milliseconds` to `net.http_post` (30s for balances, 120s for the resync). Its default is **5000ms**, which neither job originally set — production showed six timed-out calls to one recorded success, and the resync that recovered the Aug 15–19 outage took 27 seconds. The work still completed (the Edge Function runs to the end and returns 200 even after pg_net hangs up), but `net._http_response` recorded a timeout on every run, healthy or not — so the one place that would say "the scheduled resync is failing" always read the same. `plaid-sync-health-check` makes no HTTP call at all and is immune by construction.
...
[`20260819200000_set_cron_http_timeouts.sql`](supabase/migrations/20260819200000_set_cron_http_timeouts.sql) (the `timeout_milliseconds` fix).
