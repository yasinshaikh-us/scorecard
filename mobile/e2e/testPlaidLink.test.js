...
// mobile/README.md's Stage 2 section). Instead, a real Plaid Sandbox bank
// is seeded by e2e/globalSetup.js, which calls the
// supabase/functions/test-plaid-link Edge Function before the emulator is
// booted -- it mints a real Sandbox public_token via Plaid's own
// /sandbox/public_token/create endpoint and runs it through the REAL
// plaid-exchange + syncItemTransactions pipeline. This spec then asserts
// the part of the flow the app actually owns: that it reads that linked
// account back under RLS and renders it.
//
// The seeding used to happen through a "Link test bank" button in the app
// itself, which this spec tapped. That button is gone -- see
// e2e/testAccount.js's seedSandboxBank for what it cost and why nothing
// this spec verifies depended on it. Seeding before the app runs also
// removes the PlaidLinkGate detour this spec used to take: the account has
// a bank by the time the app starts, so a signed-in session lands on Home.
